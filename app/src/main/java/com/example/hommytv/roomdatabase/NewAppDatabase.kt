...
@Database(entities = [FavTable::class,WatchLaterTable::class],version=200)
abstract class NewAppDatabase(): RoomDatabase(){
    abstract fun databaseMethods():DatabaseMethods

    companion object{
        @Volatile
        var appDatabaseObject:NewAppDatabase?=null

        fun getAppDatabaseObject(context: Context):NewAppDatabase{

            return  appDatabaseObject?: synchronized(this){

                val appDatabaseObj= Room.databaseBuilder(context,NewAppDatabase::class.java,"App_Database").addMigrations(migrationO).build()
